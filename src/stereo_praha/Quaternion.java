...
public final class Quaternion {
    ...
    public Quaternion(final double scalar,
        ...
        throws Exception {
        if (v.length != 3) {
            throw new Exception("expected 3 components in argument. got " + v.length);
        }
        this.q0 = scalar;
        this.q1 = v[0];
        this.q2 = v[1];
     ...
     */
    public Quaternion(final double[] v) throws Exception {
        this(0, v);
    }
    ...
    static double PRACTICAL_ZERO = 0.000000000001;
    /**
     ...
     */
    public Quaternion normalize() throws Exception {
        final double norm = getNorm();

        if (norm < PRACTICAL_ZERO) {
            throw new Exception("norm is zero");
        }
    ...
    public boolean equals(final Quaternion q,
        ...
        double d0 = Math.abs(q0 -  q.getQ0());
        double d1 = Math.abs(q1 -  q.getQ1());
        double d2 = Math.abs(q2 -  q.getQ2());
    ...
    public boolean isUnitQuaternion(double eps) {
        ...
        return Math.abs(getNorm() - 1d) < eps;
    }
...
//    }
     ...
     */
    public Quaternion getInverse() throws Exception {
        final double squareNorm = q0 * q0 + q1 * q1 + q2 * q2 + q3 * q3;
        if (squareNorm < PRACTICAL_ZERO) {
            throw new Exception("squareNorm can't be zero");
        }
    ...
    public static Quaternion fromEuler(double ax, double ay, double az) // roll (x), pitch (Y), yaw (z)
    {
        // Abbreviations for the various angular functions

        double cr = Math.cos(ax * 0.5);
        double sr = Math.sin(ax * 0.5);
        double cp = Math.cos(ay * 0.5);
        double sp = Math.sin(ay * 0.5);
        double cy = Math.cos(az * 0.5);
        double sy = Math.sin(az * 0.5);
